...
import { Shield, EyeOff, Volume2, StopCircle, Settings, X, RefreshCw, Eye, ZoomIn, ZoomOut, Sun, Moon, Contrast } from 'lucide-react';

interface NoteViewerProps {
  fileUrl: string; // In real app, this is the PDF page image source
  buyerPhone: string;
  buyerName: string;
  textContent?: string; // Content for the audio player
}
...
const NoteViewer: React.FC<NoteViewerProps> = ({ fileUrl, buyerPhone, buyerName, textContent }) => {
  ...
  // 1. Load Image (Only once when fileUrl changes)
  useEffect(() => {
    setIsLoaded(false);
    const img = new Image();
  ...
  // 2. Draw Canvas (Re-runs when image or settings change)
  useEffect(() => {
    ...
    // Set Canvas Dimensions to Image
    ...
    // Loop to cover the whole canvas
  ...
  // 3. Audio Player Logic (Web Speech API)
  useEffect(() => {
  ...
  const toggleAudio = () => {
    ...
    if (isPlaying) {
      ...
      utteranceRef.current.rate = 1.1; // Slightly faster for efficiency
  ...
  // 4. Security: Disable Context Menu (Right Click)
  ...
  const getFilterStyle = () => {
    switch(visualFilter) {
      ...
      case 'contrast': return 'contrast(1.5) grayscale(1)';
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        {/* Canvas wrapper for zoom logic */}
        <div 
            style={{ 
                width: isLoaded ? `${image?.width ? image.width * zoom : 100}%` : '100%',
                maxWidth: 'none', // Allow overflow for zoom
        ...
        {/* Overlay to further discourage screenshots/scraping */}
